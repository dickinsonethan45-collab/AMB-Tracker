...
"connections" scope, which requires the user to click through an
authorize screen once. That's what /link in bot.py sends them to.
"""
...
<style>
  ...
  .box {{ background:#2b2d31; padding:32px 40px; border-radius:12px; max-width:420px; }}
...
async def handle_callback(request: web.Request):
    ...
    if not code or not state or state not in _pending_states:
        return web.Response(
            text=_page("Link failed", "That link expired or is invalid — run /link again in Discord."),
    ...
    if pending["expires"] < time.time():
        return web.Response(
            text=_page("Link expired", "That link expired — run /link again in Discord."),
            content_type="text/html",
            status=400,
        )
    ...
    async with ClientSession() as session:
        try:
            ...
            return web.Response(
                text=_page("Link failed", "Something went wrong talking to Discord — try /link again."),
                content_type="text/html",
                status=500,
            )
    ...
    if youtube_name:
        ...
        found.append(f"YouTube ({youtube_name})")
    if tiktok_name:
        entry["tiktok_username"] = tiktok_name
        found.append(f"TikTok ({tiktok_name})")
    storage.save(data)

    if not found:
        return web.Response(
            text=_page(
                "No YouTube/TikTok found",
                "You authorized, but I didn't see a YouTube or TikTok connection on your Discord "
                "account. Add one under Discord Settings -> Connections, then run /link again.",
            ),
            content_type="text/html",
        )

    return web.Response(
        text=_page("Linked!", f"Connected: {', '.join(found)}. You can close this tab and go back to Discord."),
        content_type="text/html",
    )
